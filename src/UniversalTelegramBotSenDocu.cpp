/*
   ...
   UniversalTelegramBotSenDocu - Library to create your own Telegram Bot using
...
UniversalTelegramBotSenDocu::UniversalTelegramBotSenDocu(const String& token, Client &client) {
  updateToken(token);
  this->client = &client;
}

void UniversalTelegramBotSenDocu::updateToken(const String& token) {
  _token = token;
}

String UniversalTelegramBotSenDocu::getToken() {
  return _token;
}

String UniversalTelegramBotSenDocu::buildCommand(const String& cmd) {
  String command;
...
String UniversalTelegramBotSenDocu::sendGetToTelegram(const String& command) {
  String body, headers;
...
bool UniversalTelegramBotSenDocu::readHTTPAnswer(String &body, String &headers) {
  int ch_count = 0;
...
String UniversalTelegramBotSenDocu::sendPostToTelegram(const String& command, JsonObject payload) {
...
String UniversalTelegramBotSenDocu::sendMultipartFormDataToTelegram(
...
bool UniversalTelegramBotSenDocu::getMe() {
 ...
 ********************************************************************************/
bool UniversalTelegramBotSenDocu::setMyCommands(const String& commandArray) {
 ...
 ***************************************************************/
int UniversalTelegramBotSenDocu::getUpdates(long offset) {
...
bool UniversalTelegramBotSenDocu::processResult(JsonObject result, int messageIndex) {
 ...
 ***********************************************************************/
bool UniversalTelegramBotSenDocu::sendSimpleMessage(const String& chat_id, const String& text,
...
bool UniversalTelegramBotSenDocu::sendMessage(const String& chat_id, const String& text,
...
bool UniversalTelegramBotSenDocu::sendMessageWithReplyKeyboard(
...
bool UniversalTelegramBotSenDocu::sendMessageWithInlineKeyboard(const String& chat_id,
                                                         const String& text,
                                                         const String& parse_mode,
 ...
 * (Arguments to pass: chat_id, text to transmit and markup(optional)) *
 ***********************************************************************/
bool UniversalTelegramBotSenDocu::sendPostMessage(JsonObject payload, bool edit) { // added message_id
...
String UniversalTelegramBotSenDocu::sendPostPhoto(JsonObject payload) {
...
String UniversalTelegramBotSenDocu::sendDocument(const String& chat_id,const String& fileName,const String& contentType, int fileSize,MoreDataAvailable moreDataAvailableCallback,
    GetNextByte getNextByteCallback, GetNextBuffer getNextBufferCallback, GetNextBufferLen getNextBufferLenCallback) 
{
  ...
  String response = sendMultipartFormDataToTelegram("sendDocument", "document", fileName,contentType, chat_id, fileSize,
...
String UniversalTelegramBotSenDocu::sendPhotoByBinary(
    const String& chat_id, const String& fileName,const String& contentType, int fileSize,
    MoreDataAvailable moreDataAvailableCallback,
    GetNextByte getNextByteCallback, GetNextBuffer getNextBufferCallback, GetNextBufferLen getNextBufferLenCallback) 
{
  ...
  String response = sendMultipartFormDataToTelegram("sendPhoto", "photo", fileName,
...
String UniversalTelegramBotSenDocu::sendPhoto(const String& chat_id, const String& photo,
...
bool UniversalTelegramBotSenDocu::checkForOkResponse(const String& response) {
  int last_id;
...
bool UniversalTelegramBotSenDocu::sendChatAction(const String& chat_id, const String& text) {
...
void UniversalTelegramBotSenDocu::closeClient() {
  if (client->connected()) {
    #ifdef TELEGRAM_DEBUG  
...
bool UniversalTelegramBotSenDocu::getFile(String& file_path, long& file_size, const String& file_id)
{
...
bool UniversalTelegramBotSenDocu::answerCallbackQuery(const String &query_id, const String &text, bool show_alert, const String &url, int cache_time) {
  DynamicJsonDocument payload(maxMessageLength);
